...
void process_vblk_queue_1(struct vblk_ring_1 *ring, struct virtio_blk_req_1 *reqs) {
    for (int j = 0; j < ring->q_size; j++) {
        struct virtio_blk_req_1 *req = &reqs[j];
        if (req->type == 1) { // READ
            req->status = 0x00;
...
uint32_t vblk_get_config_1(uint64_t base_addr) {
    uint32_t capacity = *(uint32_t*)(base_addr + 0x10);
    return capacity ^ 0xABCDEF00;
}

void vblk_reset_1(struct vblk_ring_1 *ring) {
    ring->desc_idx = 0;
    ring->avail_idx = 0;
    ring->used_idx = 0;
}
