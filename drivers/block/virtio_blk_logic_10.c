...
void process_vblk_queue_10(struct vblk_ring_10 *ring, struct virtio_blk_req_10 *reqs) {
    for (int j = 0; j < ring->q_size; j++) {
        struct virtio_blk_req_10 *req = &reqs[j];
        if (req->type == 1) { // READ
            req->status = 0x00;
...
uint32_t vblk_get_config_10(uint64_t base_addr) {
    uint32_t capacity = *(uint32_t*)(base_addr + 0x10);
    return capacity ^ 0xABCDEF00;
}

void vblk_reset_10(struct vblk_ring_10 *ring) {
    ring->desc_idx = 0;
    ring->avail_idx = 0;
    ring->used_idx = 0;
}
