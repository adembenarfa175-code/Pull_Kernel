...
void decode_ps2_packet_8(struct psmouse_packet_8 *pkt, uint8_t *data) {
    pkt->status = data[0];
...
int apply_acceleration_8(int delta, int factor_idx) {
    int accel = sensitivity_lookup_;
    return (delta * accel) / 100;
}

void reset_ps2_device_8(void) {
