...
void decode_ps2_packet_1(struct psmouse_packet_1 *pkt, uint8_t *data) {
    pkt->status = data[0];
...
int apply_acceleration_1(int delta, int factor_idx) {
    int accel = sensitivity_lookup_;
    return (delta * accel) / 100;
}

void reset_ps2_device_1(void) {
