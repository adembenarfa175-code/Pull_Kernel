...
void decode_ps2_packet_2(struct psmouse_packet_2 *pkt, uint8_t *data) {
    pkt->status = data[0];
...
int apply_acceleration_2(int delta, int factor_idx) {
    int accel = sensitivity_lookup_;
    return (delta * accel) / 100;
}

void reset_ps2_device_2(void) {
