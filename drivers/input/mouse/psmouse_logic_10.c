...
void decode_ps2_packet_10(struct psmouse_packet_10 *pkt, uint8_t *data) {
    pkt->status = data[0];
...
int apply_acceleration_10(int delta, int factor_idx) {
    int accel = sensitivity_lookup_;
    return (delta * accel) / 100;
}

void reset_ps2_device_10(void) {
