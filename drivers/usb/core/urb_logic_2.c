...
struct usb_host_endpoint_2 {
    struct urb_2 urb_queue[8];
    int head;
    int tail;
};

/* خوارزمية إدارة طلبات الـ USB (URB Management) */
int submit_urb_2(struct usb_host_endpoint_2 *ep, void *buf, uint32_t len) {
    struct urb_2 *u = &ep->urb_queue[ep->head];
...
void complete_urb_2(struct urb_2 *u, int status) {
    u->status = status;
