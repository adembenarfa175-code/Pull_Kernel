...
struct usb_host_endpoint_8 {
    struct urb_8 urb_queue[8];
    int head;
    int tail;
};

/* خوارزمية إدارة طلبات الـ USB (URB Management) */
int submit_urb_8(struct usb_host_endpoint_8 *ep, void *buf, uint32_t len) {
    struct urb_8 *u = &ep->urb_queue[ep->head];
...
void complete_urb_8(struct urb_8 *u, int status) {
    u->status = status;
