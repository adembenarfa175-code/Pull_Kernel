...
struct usb_host_endpoint_12 {
    struct urb_12 urb_queue[8];
    int head;
    int tail;
};

/* خوارزمية إدارة طلبات الـ USB (URB Management) */
int submit_urb_12(struct usb_host_endpoint_12 *ep, void *buf, uint32_t len) {
    struct urb_12 *u = &ep->urb_queue[ep->head];
...
void complete_urb_12(struct urb_12 *u, int status) {
    u->status = status;
