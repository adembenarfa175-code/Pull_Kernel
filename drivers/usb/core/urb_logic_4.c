...
struct usb_host_endpoint_4 {
    struct urb_4 urb_queue[8];
    int head;
    int tail;
};

/* خوارزمية إدارة طلبات الـ USB (URB Management) */
int submit_urb_4(struct usb_host_endpoint_4 *ep, void *buf, uint32_t len) {
    struct urb_4 *u = &ep->urb_queue[ep->head];
...
void complete_urb_4(struct urb_4 *u, int status) {
    u->status = status;
