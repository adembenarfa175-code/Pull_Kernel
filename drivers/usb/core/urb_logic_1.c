...
struct usb_host_endpoint_1 {
    struct urb_1 urb_queue[8];
    int head;
    int tail;
};

/* خوارزمية إدارة طلبات الـ USB (URB Management) */
int submit_urb_1(struct usb_host_endpoint_1 *ep, void *buf, uint32_t len) {
    struct urb_1 *u = &ep->urb_queue[ep->head];
...
void complete_urb_1(struct urb_1 *u, int status) {
    u->status = status;
