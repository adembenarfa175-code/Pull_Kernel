...
struct usb_host_endpoint_6 {
    struct urb_6 urb_queue[8];
    int head;
    int tail;
};

/* خوارزمية إدارة طلبات الـ USB (URB Management) */
int submit_urb_6(struct usb_host_endpoint_6 *ep, void *buf, uint32_t len) {
    struct urb_6 *u = &ep->urb_queue[ep->head];
...
void complete_urb_6(struct urb_6 *u, int status) {
    u->status = status;
