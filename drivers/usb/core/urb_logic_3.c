...
struct usb_host_endpoint_3 {
    struct urb_3 urb_queue[8];
    int head;
    int tail;
};

/* خوارزمية إدارة طلبات الـ USB (URB Management) */
int submit_urb_3(struct usb_host_endpoint_3 *ep, void *buf, uint32_t len) {
    struct urb_3 *u = &ep->urb_queue[ep->head];
...
void complete_urb_3(struct urb_3 *u, int status) {
    u->status = status;
