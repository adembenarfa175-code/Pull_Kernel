...
struct usb_host_endpoint_9 {
    struct urb_9 urb_queue[8];
    int head;
    int tail;
};

/* خوارزمية إدارة طلبات الـ USB (URB Management) */
int submit_urb_9(struct usb_host_endpoint_9 *ep, void *buf, uint32_t len) {
    struct urb_9 *u = &ep->urb_queue[ep->head];
...
void complete_urb_9(struct urb_9 *u, int status) {
    u->status = status;
