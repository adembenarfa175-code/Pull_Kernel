...
struct usb_host_endpoint_10 {
    struct urb_10 urb_queue[8];
    int head;
    int tail;
};

/* خوارزمية إدارة طلبات الـ USB (URB Management) */
int submit_urb_10(struct usb_host_endpoint_10 *ep, void *buf, uint32_t len) {
    struct urb_10 *u = &ep->urb_queue[ep->head];
...
void complete_urb_10(struct urb_10 *u, int status) {
    u->status = status;
