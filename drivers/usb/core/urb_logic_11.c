...
struct usb_host_endpoint_11 {
    struct urb_11 urb_queue[8];
    int head;
    int tail;
};

/* خوارزمية إدارة طلبات الـ USB (URB Management) */
int submit_urb_11(struct usb_host_endpoint_11 *ep, void *buf, uint32_t len) {
    struct urb_11 *u = &ep->urb_queue[ep->head];
...
void complete_urb_11(struct urb_11 *u, int status) {
    u->status = status;
