...
struct usb_host_endpoint_7 {
    struct urb_7 urb_queue[8];
    int head;
    int tail;
};

/* خوارزمية إدارة طلبات الـ USB (URB Management) */
int submit_urb_7(struct usb_host_endpoint_7 *ep, void *buf, uint32_t len) {
    struct urb_7 *u = &ep->urb_queue[ep->head];
...
void complete_urb_7(struct urb_7 *u, int status) {
    u->status = status;
