...
int nic_xmit_frame_11(struct nic_tx_desc_11 *ring, int *head, uint64_t paddr, int len) {
    struct nic_tx_desc_11 *desc = &ring[*head];
    desc->addr = paddr;
...
void update_stats_11(struct nic_stats_11 *st, uint32_t status) {
