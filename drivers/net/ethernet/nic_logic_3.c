...
int nic_xmit_frame_3(struct nic_tx_desc_3 *ring, int *head, uint64_t paddr, int len) {
    struct nic_tx_desc_3 *desc = &ring[*head];
    desc->addr = paddr;
...
void update_stats_3(struct nic_stats_3 *st, uint32_t status) {
