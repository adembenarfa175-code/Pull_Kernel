...
int nic_xmit_frame_4(struct nic_tx_desc_4 *ring, int *head, uint64_t paddr, int len) {
    struct nic_tx_desc_4 *desc = &ring[*head];
    desc->addr = paddr;
...
void update_stats_4(struct nic_stats_4 *st, uint32_t status) {
