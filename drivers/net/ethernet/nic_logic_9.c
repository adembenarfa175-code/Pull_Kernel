...
int nic_xmit_frame_9(struct nic_tx_desc_9 *ring, int *head, uint64_t paddr, int len) {
    struct nic_tx_desc_9 *desc = &ring[*head];
    desc->addr = paddr;
...
void update_stats_9(struct nic_stats_9 *st, uint32_t status) {
