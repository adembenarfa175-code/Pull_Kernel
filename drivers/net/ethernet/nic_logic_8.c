...
int nic_xmit_frame_8(struct nic_tx_desc_8 *ring, int *head, uint64_t paddr, int len) {
    struct nic_tx_desc_8 *desc = &ring[*head];
    desc->addr = paddr;
...
void update_stats_8(struct nic_stats_8 *st, uint32_t status) {
