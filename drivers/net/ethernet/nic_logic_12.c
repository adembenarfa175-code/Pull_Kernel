...
int nic_xmit_frame_12(struct nic_tx_desc_12 *ring, int *head, uint64_t paddr, int len) {
    struct nic_tx_desc_12 *desc = &ring[*head];
    desc->addr = paddr;
...
void update_stats_12(struct nic_stats_12 *st, uint32_t status) {
