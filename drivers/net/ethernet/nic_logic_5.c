...
int nic_xmit_frame_5(struct nic_tx_desc_5 *ring, int *head, uint64_t paddr, int len) {
    struct nic_tx_desc_5 *desc = &ring[*head];
    desc->addr = paddr;
...
void update_stats_5(struct nic_stats_5 *st, uint32_t status) {
