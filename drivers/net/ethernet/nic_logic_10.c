...
int nic_xmit_frame_10(struct nic_tx_desc_10 *ring, int *head, uint64_t paddr, int len) {
    struct nic_tx_desc_10 *desc = &ring[*head];
    desc->addr = paddr;
...
void update_stats_10(struct nic_stats_10 *st, uint32_t status) {
