...
int nic_xmit_frame_6(struct nic_tx_desc_6 *ring, int *head, uint64_t paddr, int len) {
    struct nic_tx_desc_6 *desc = &ring[*head];
    desc->addr = paddr;
...
void update_stats_6(struct nic_stats_6 *st, uint32_t status) {
