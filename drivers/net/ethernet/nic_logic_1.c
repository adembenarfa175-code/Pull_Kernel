...
int nic_xmit_frame_1(struct nic_tx_desc_1 *ring, int *head, uint64_t paddr, int len) {
    struct nic_tx_desc_1 *desc = &ring[*head];
    desc->addr = paddr;
...
void update_stats_1(struct nic_stats_1 *st, uint32_t status) {
