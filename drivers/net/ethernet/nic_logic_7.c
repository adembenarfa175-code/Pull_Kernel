...
int nic_xmit_frame_7(struct nic_tx_desc_7 *ring, int *head, uint64_t paddr, int len) {
    struct nic_tx_desc_7 *desc = &ring[*head];
    desc->addr = paddr;
...
void update_stats_7(struct nic_stats_7 *st, uint32_t status) {
