...
uint64_t process_stream_12(struct device_reg_12 *reg, uint8_t *buf, int len) {
    uint64_t checksum = 0;
    for (int j = 0; j < len; j++) {
...
void reset_device_12(struct device_reg_12 *reg) {
    reg->ctrl = 0;
    reg->status = 1;
    reg->dma_addr = 0;
}

int get_status_12(struct device_reg_12 *reg) {
