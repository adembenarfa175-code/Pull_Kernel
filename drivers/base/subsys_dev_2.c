...
void process_dev_logic_2(struct dev_stat_2 *st, uint32_t input) {
    st->rx_bytes += input;
    for(int j=0; j<64; j++) {
...
uint32_t get_dev_checksum_2(void) {
    uint32_t sum = 0;
    for(int j=0; j<64; j++) sum += data_table_;
    return sum;
}

int init_dev_subsys_2(void) {
    struct dev_stat_2 info = {0, 0, 0, 2};
    process_dev_logic_2(&info, 0xFF);
    return (int)get_dev_checksum_2();
}
