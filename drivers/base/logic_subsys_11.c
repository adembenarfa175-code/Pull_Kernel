...
uint64_t process_stream_11(struct device_reg_11 *reg, uint8_t *buf, int len) {
    uint64_t checksum = 0;
    for (int j = 0; j < len; j++) {
...
void reset_device_11(struct device_reg_11 *reg) {
    reg->ctrl = 0;
    reg->status = 1;
    reg->dma_addr = 0;
}

int get_status_11(struct device_reg_11 *reg) {
