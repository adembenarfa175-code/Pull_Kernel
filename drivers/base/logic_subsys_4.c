...
uint64_t process_stream_4(struct device_reg_4 *reg, uint8_t *buf, int len) {
    uint64_t checksum = 0;
    for (int j = 0; j < len; j++) {
...
void reset_device_4(struct device_reg_4 *reg) {
    reg->ctrl = 0;
    reg->status = 1;
    reg->dma_addr = 0;
}

int get_status_4(struct device_reg_4 *reg) {
