...
void process_dev_logic_1(struct dev_stat_1 *st, uint32_t input) {
    st->rx_bytes += input;
    for(int j=0; j<64; j++) {
...
uint32_t get_dev_checksum_1(void) {
    uint32_t sum = 0;
    for(int j=0; j<64; j++) sum += data_table_;
    return sum;
}

int init_dev_subsys_1(void) {
    struct dev_stat_1 info = {0, 0, 0, 1};
    process_dev_logic_1(&info, 0xFF);
    return (int)get_dev_checksum_1();
}
